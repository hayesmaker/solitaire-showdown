define(
  ...
  function(_, Class, Phaser, TweenMax, PIXI, SpecialPileView, Mocks) {

    var view, game;

    describe("SpecialPileView.View tests", function() {

      beforeEach(function() {
        view = new SpecialPileView();
        game = Mocks.mockGame;
        view.init(game);
      });
      ...
      it("SpecialPileView is initialised correctly", function() {
        expect(view).toBeDefined();
      });

      it("Phaser.Game is initialised", function() {
        expect(view.game).toBeDefined();
      });

      describe("When drawStack is called, An empty stack should be drawn to the screen", function() {
